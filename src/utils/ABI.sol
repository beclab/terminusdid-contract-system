...
library ABI {
    ...
    function uintT(uint16 bits) internal pure returns (bytes2) {
        if (bits & 0x7 == 0) {
            uint16 size = bits >> 3;
            if (size >= 1 && size <= 32) {
    ...
    function boolT() internal pure returns (bytes1) {
        return BOOL_T;
    }

    function stringT() internal pure returns (bytes1) {
        return STRING_T;
    }

    function addressT() internal pure returns (bytes1) {
        return ADDRESS_T;
    }

    function bytesT(uint8 size) internal pure returns (bytes2) {
        if (size >= 1 && size <= 32) {
    ...
    function bytesT() internal pure returns (bytes1) {
        return BYTES_T;
    }

    function arrayT(bytes memory elem, uint16 count) internal pure returns (bytes memory) {
        validateType(elem);
        if (count == 0) {
            revert InvalidType();
        }
    ...
    function arrayT(bytes memory elem) internal pure returns (bytes memory) {
        validateType(elem);
        return bytes.concat(DYNAMIC_ARRAY_T, elem);
    }

    function tupleT(bytes memory elems) internal pure returns (bytes memory) {
        uint256 count = 0;
    ...
    function validateType(bytes memory typ) internal pure {
    ...
    function at(ReflectVar memory self, uint256 index) internal view returns (ReflectVar memory) {
    ...
    function get(ReflectVar memory self) internal view returns (bytes memory) {
    ...
    function length(ReflectVar memory self) internal view returns (uint256) {
    ...
    function pop(ReflectVar memory self) internal {
        (bytes32 slot,) = _align(self.slot, self.offset);

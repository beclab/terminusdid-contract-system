...
contract TerminusDIDTest is Test {
    ...
    function testSetTagger() public {
        ...
        // msg.sender is not operator nor domain owner
        vm.expectRevert(TerminusDID.Unauthorized.selector);
        terminusDIDProxy.setTagger(rootDomain, tagName, tagger);
    ...
    function testAddRemoveUpdateOfficialTag() public {
        ...
        vm.prank(_operator);
        terminusDIDProxy.defineTag(rootDomain, tagName, ipV4Type, fieldNames);
        ...
        bytes memory gotData = terminusDIDProxy.getTagElem(rootDomain, domain, tagName, elePaths);
        ...
        vm.prank(tagger);
        terminusDIDProxy.updateTagElem(rootDomain, domain, tagName, elePaths, newData);

        // get tag new value
        bytes memory gotNewData = terminusDIDProxy.getTagElem(rootDomain, domain, tagName, elePaths);
        ...
        vm.expectRevert(TerminusDID.Unauthorized.selector);
        terminusDIDProxy.removeTag(rootDomain, domain, tagName);
        ...
        terminusDIDProxy.getTagElem(rootDomain, domain, tagName, elePaths);
    ...
    function testAddTagWithoutTagTypeDefinition() public {
        ...
        bytes memory data = abi.encode(ipV4Address);
        vm.prank(tagger);
        vm.expectRevert(abi.encodeWithSelector(TagRegistry.UndefinedTag.selector, rootDomain, tagName));
        terminusDIDProxy.addTag(rootDomain, domain, tagName, data);
    }
    ...
    function testAddMultipleTags() public {
        ...
        bytes memory addressArrayType = ABI.arrayT(bytes.concat(ABI.addressT()));
        vm.prank(_operator);
        terminusDIDProxy.defineTag(rootDomain, authAddressesTagName, addressArrayType, fieldNames);
        ...
        // define tagger for rootDomain and the above 3 tags
        address tagger = address(100);
        vm.prank(_operator);
        terminusDIDProxy.setTagger(rootDomain, authAddressesTagName, tagger);
        vm.prank(_operator);
        terminusDIDProxy.setTagger(rootDomain, rsaPubKeyTagName, tagger);
        vm.prank(_operator);
        terminusDIDProxy.setTagger(rootDomain, dnsARecordTagName, tagger);
        ...
        vm.prank(tagger);
        terminusDIDProxy.addTag(rootDomain, domain, authAddressesTagName, data);
        ...
        vm.prank(tagger);
        terminusDIDProxy.addTag(rootDomain, domain, rsaPubKeyTagName, data);

        bytes4 ipV4Address = bytes4(hex"ffffffff");
        data = abi.encode(ipV4Address);
        vm.prank(tagger);
        terminusDIDProxy.addTag(rootDomain, domain, dnsARecordTagName, data);

        uint256 tagCount = terminusDIDProxy.getTagCount(rootDomain, domain);
        assertEq(tagCount, 3);

        uint256[] memory elemPath;
        for (uint256 i = 0; i < tagCount; i++) {
            string memory tagName = terminusDIDProxy.getTagNameByIndex(rootDomain, domain, i);
            if (i == 0) {
                assertEq(tagName, authAddressesTagName);
                bytes memory gotData = terminusDIDProxy.getTagElem(rootDomain, domain, tagName, elemPath);
    ...
    function testFuzzErc721TransferBySuperOperator(bool allowSubdomain) public {
        string memory domain = "com";
        string memory did = "did";
    ...
    function testErc721TransferByParentDomainOwner() public {
        string memory domain = "com";
        string memory did = "did";

        address ownerParent = address(100);
        TerminusDID.Metadata memory metadata = TerminusDID.Metadata(domain, did, "", true);
        vm.prank(_operator);
        uint256 tokenIdParent = terminusDIDProxy.register(ownerParent, metadata);

        assertEq(terminusDIDProxy.ownerOf(tokenIdParent), ownerParent);

        address owner = address(200);
        vm.prank(_operator);
        uint256 tokenId = terminusDIDProxy.register(owner, TerminusDID.Metadata("bytetrade.com", did, "", true));

        assertEq(terminusDIDProxy.ownerOf(tokenId), owner);

        address receiver = address(300);

        vm.prank(ownerParent);
        vm.expectEmit(true, true, true, false);
        emit Transfer(owner, receiver, tokenId);
        terminusDIDProxy.transferFrom(owner, receiver, tokenId);

...
const { BigNumber, utils, getContractFactory } = ethers;
...
describe('RsaPubKeyResolver test', function () {
    ...
    async function canParseRsaPubKeyWithDifferentLength(length) {
        ...
        expect(await rsaPubKeyResolver.rsaPubKeyResolverValidate(pubKeyDer)).equal(0);
        const [status, retBytes] = await rsaPubKeyResolver.rsaPubKeyResolverParse(pubKeyDer);
        expect(status).equal(0);

        const abiDecoder = new utils.AbiCoder();
        const [modulus, publicExponent] = abiDecoder.decode(["bytes", "uint256"], retBytes);

        expect(modulus).to.equal(modulusFromJwk);
        expect(publicExponent).to.equal(publicExponentFromJwk);
    }

    it('contract can rsaPubKeyResolverParse rsa pubkey to the same result with nodejs crypto package', async function () {
        let testTimes = 5;
        while (testTimes--) {
    ...
    describe('invalid pubKey test', async function () {
        ...
        it('invalid sequence length', async function () {
            ...
            const { rsaPubKeyResolver } = await loadFixture(deployTokenFixture);
            expect(await rsaPubKeyResolver.rsaPubKeyResolverValidate(pubKeyDer)).equal(5);
        });
